...
temp <- tempfile()
source <- "https://www.england.nhs.uk/statistics/wp-content/uploads/sites/2/2020/10/COVID-19-total-announced-deaths-7-October-2020.xlsx"
temp <- curl_download(url=source, destfile=temp, quiet=FALSE, mode="wb")

deaths<-as.data.table(read_excel(temp, sheet=6, col_names = F))

deaths<-deaths[18:.N, c(1:228)]

deaths<- melt.data.table(deaths, id=1:4, measure.vars = 5:228)
