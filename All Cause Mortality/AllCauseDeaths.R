...
temp <- tempfile()
source <- "https://www.england.nhs.uk/statistics/wp-content/uploads/sites/2/2020/07/COVID-19-total-announced-deaths-20-July-2020.xlsx"
temp <- curl_download(url=source, destfile=temp, quiet=FALSE, mode="wb")

deaths<-as.data.table(read_excel(temp, sheet=6, col_names = F))

deaths<-deaths[18:.N, c(1:149)]

deaths<- melt.data.table(deaths, id=1:4, measure.vars = 5:149)
...
deathtiles <- ggplot(heatmap, aes(x=date, y=fct_reorder(name, maxdeathsday), fill=maxdeathprop))+
  ...
  scale_x_date(name="Date", limits=as.Date(c(plotfrom_d, plotto)), expand=c(0,0))+
