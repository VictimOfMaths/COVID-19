...
EWDate <- "20th November"
ScotDate <- "29th November"
NIDate <- "27th November"

#Locations for latest data. Links for historical data don't move, so keep them further down
Eng2020 <- "https://www.ons.gov.uk/file?uri=%2fpeoplepopulationandcommunity%2fbirthsdeathsandmarriages%2fdeaths%2fdatasets%2fweeklyprovisionalfiguresondeathsregisteredinenglandandwales%2f2020/publishedweek472020.xlsx"
...
EngRange <- "AW" #increment by one letter each week
ScotRange <- "AX" #
...
NIRange <- "52" #increment by one number each week
...
ggplot()+
  ...
  expand_limits(y=0)+
  labs(title="All-cause deaths in England & Wales continue to rise",
...
ggplot()+
  ...
  scale_y_continuous(name="Deaths registered")+
  expand_limits(y=0)+
  labs(title="The increase in deaths has affected both men and women",
       subtitle=paste0("Weekly deaths in <span style='color:red;'>2020</span> compared to <span style='color:Skyblue4;'>the range in 2010-19</span>. Data up to ", EWDate, "."),
       caption="Data from ONS | Plot by @VictimOfMaths")+
...
ggplot()+
  ...
  scale_y_continuous(name="Deaths registered")+
  expand_limits(y=0)+
  labs(title="The rise in deaths is evident across all age groups",
       subtitle=paste0("Weekly deaths in <span style='color:red;'>2020</span> compared to <span style='color:Skyblue4;'>the range in 2010-19</span>. Data up to ", EWDate, "."),
       caption="Data from ONS | Plot by @VictimOfMaths")+
  theme(strip.background=element_blank(), strip.text=element_text(face="bold", size=rel(1)),
        plot.subtitle =element_markdown())+
...
ggplot()+
  ...
  scale_y_continuous(name="Deaths registered")+
  expand_limits(y=0)+
  labs(title="All-cause mortality appears to have levelled off in the under 75s",
       subtitle=paste0("Weekly deaths in <span style='color:red;'>2020</span> compared to <span style='color:Skyblue4;'>the range in 2010-19</span>. Data up to ", EWDate, "."),
       caption="Data from ONS | Plot by @VictimOfMaths")+
  theme(strip.background=element_blank(), strip.text=element_text(face="bold", size=rel(1)),
        plot.subtitle =element_markdown())+
...
ggplot()+
  ...
  scale_y_continuous(name="Deaths registered")+
  expand_limits(y=0)+
  labs(title="All-cause deaths in Scotland have started to fall again",
...
data20 <- bind_rows(temp2, temp3, temp4, temp5, temp6, temp7, temp8, temp9, temp10, temp11, 
                    ...
                    temp30, temp31, temp32, temp33, temp34, temp35, temp36, temp37)
...
ggplot()+
  ...
  labs(title="The rise in mortality has happened almost entirely in hospitals",
...
ggplot()+
  ...
  theme_classic()+
  labs(title="The increase in all-cause deaths is entirely due to COVID-19",
