...
temp <- tempfile()
source <- "https://www.nrscotland.gov.uk/files//statistics/covid19/covid-deaths-data-week-32.xlsx"
...
ggplot(subset(data, loc!="All" & loc!="Other"))+
  ...
  theme_classic()+
  theme(strip.background=element_blank(), strip.text=element_text(face="bold", size=rel(1)))+
  labs(title="Elevated levels of home mortality may be displaced cancer and circulatory deaths",
