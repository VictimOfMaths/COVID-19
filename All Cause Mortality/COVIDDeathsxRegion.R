...
deaths.reg %>% 
  ...
  scale_y_continuous(name="Daily COVID-19 deaths per 100,000")+
  scale_colour_paletteer_d("LaCroixColoR::paired", name="Region")+
  labs(title="Deaths associated with COVID-19 look like they might be beginning to fall",
       subtitle="Daily deaths per 100,000 within 28 days of a positive COVID-19 test",
       caption="Data from PHE | Plot by @VictimOfMaths")+
  theme(plot.title=element_text(face="bold", size=rel(1.2)))
dev.off()
...
deaths.nat %>% 
  ...
  labs(title="Deaths linked to COVID-19 are falling, except in Northern Ireland",
       subtitle="Daily deaths per 100,000 within 28 days of a positive COVID-19 test",
       caption="Data from PHE | Plot by @VictimOfMaths")+
  theme(plot.title=element_text(face="bold", size=rel(1.2)))
dev.off()
...
deaths.nat %>% 
  ...
  scale_y_continuous(name="Daily COVID-19 deaths per 100,000")+
  scale_colour_paletteer_d("fishualize::Scarus_quoyi", name="")+
  labs(title="Mortality rates have taken very different trajectories in the UK nations since October",
       subtitle="Daily deaths per 100,000 within 28 days of a positive COVID-19 test",
       caption="Data from PHE | Plot by @VictimOfMaths")+
  theme(plot.title=element_text(face="bold", size=rel(1.2)))
dev.off()
